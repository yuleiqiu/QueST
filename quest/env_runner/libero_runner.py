...
class LiberoSingleTaskRunner(LiberoRunner):
    ...
    def __init__(self,
                 env_factory,
                 benchmark_name,
                 task_id,  # NEW: specific task ID to evaluate
                 rollouts_per_env,
                 num_parallel_envs,
                 max_episode_length,
                 frame_stack=1,
        ...
        self.env_factory = env_factory
        self.benchmark_name = benchmark_name
        self.target_task_id = task_id  # Store target task ID
        ...
        # Set env_names to only the target task
        ...
        print(f"[LiberoSingleTaskRunner] Initialized for {benchmark_name} task_id={task_id}")
        print(f"[LiberoSingleTaskRunner] Target task: {self.env_names[0]}")

    def run_policy_in_env(self, env_name, policy, render=False):
        """Override to use the correct task_id for environment creation"""
        # Use the target_task_id instead of looking up env_name index
        env_id = self.target_task_id
        env_num = min(self.num_parallel_envs, self.rollouts_per_env)
        env_fn = lambda: lu.LiberoFrameStack(self.env_factory(env_id, self.benchmark), self.frame_stack)
        env = lu.LiberoVectorWrapper(env_fn, self.num_parallel_envs)
    ...
    def run_episode(self, env, env_name, policy, init_states_, env_num, render=False):
        """Override to use correct task_id for task embedding"""
        obs, info = env.reset(init_states=init_states_)
        ...
        # Use target_task_id instead of looking up env_name
